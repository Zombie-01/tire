...
export default function AdminProductsClient({
  ...
  // Initialize state with props from the Server Component
  const [products, setProducts] = useState(initialProducts);
  const [brands] = useState(initialBrands); // Brands list is static once loaded
  ...
  // Image deletion and DB mutations are handled via server API routes
  ...
  const handleCreateProduct = async () => {
    ...
    // 2. Refresh products list from the server (to include the new item)
    try {
      ...
      console.error("Error refetching products after creation:", error);
    }
  };

  // --- UI Rendering ---

  // Check if products array is empty to prevent division by zero in average price calculation
