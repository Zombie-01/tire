...
import { fetchOrders, fetchUsers, fetchProducts, fetchBrands } from '@/lib/supabase-config';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [fetchedOrders, fetchedUsers, fetchedProducts, fetchedBrands] = await Promise.all([
          fetchOrders(),
          fetchUsers(),
          fetchProducts(),
          fetchBrands(),
        ...
        <div className="bg-card rounded-lg border border-border p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-foreground mt-2">{brands.length}</p>
            </div>
          ...
          {orders.slice(0, 5).map((order) => (
            <div key={order.id} className="flex items-center justify-between p-3 bg-background rounded-lg">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{users.find((u) => u.id === order.user_id)?.name || 'Unknown User'}</p>
              </div>
