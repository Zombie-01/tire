...
import { products, brands, getActiveBanners, getBrandById } from '@/lib/database';
...
export default function Home() {
  ...
  return (
    <div className="space-y-6">
      ...
      {banners.length > 0 && (
        ...
        <div className="relative rounded-lg overflow-hidden bg-gradient-to-r from-black via-yellow-600 to-black text-white">
          ...
          <div className="relative z-10 p-6">
            ...
            <p className="text-lg mb-4 text-yellow-200">{banners[0].subtitle}</p>
            <Link
              href="/products"
      ...
      {/* Top 5 Products */}
      <section className="p-4">
        <h2 className="text-2xl font-bold text-foreground mb-4">Шилдэг 5 дугуй</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {brands.map((brand) => (
            ...
            >
              ...
              <h3 className="font-semibold text-foreground group-hover:text-yellow-500 transition-colors">{brand.name}</h3>
            </Link>
          ))}
        </div>
      </section>
